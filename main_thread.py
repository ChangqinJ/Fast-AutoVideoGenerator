...
def retry(conn:pymysql.Connection,max_retry_times:int=5,error_list:list[Exception] = []):
  status = False
  ...
  for i in range(max_retry_times):
    try:
      ...
      if e.args[0] in (0,2003,2006,2013):
        error_list = []
        if retry(cursor.connection,max_retry_times=max_retry_times,error_list=error_list) == True:
          #连接异常且重试成功
          ...
          if len(error_list) > 0:
            simple_log.log(f'Error list: {error_list}',log_path=logging_path)
          continue
      else:
